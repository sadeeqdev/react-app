...
export const NetworkMenu = () => {
  ...
  return (
    <div className="relative network-menu-container">
      ...
      >
        <div>
          <img src={env.config.ui.logo} alt="Logo" className="w-6 h-6 lg:h-7 lg:w-7" />
        </div>
          ...
          {networkList.map((network, index) => (
            <li
              ...
              className={`py-2 px-2 border-b border-gray-700 hover:bg-gray-700 hover:rounded-t-md cursor-pointer flex items-center text-sm sm:text-[16px] font-semibold gap-2 ${
                index === networkList.length - 1 ? 'last:border-none hover:last:rounded-t-none last:rounded-b-md' : ''
              }`}
            >
              <img src={network.icon} alt={`${network.name} Icon`} className="w-6 h-6 lg:h-7 lg:w-7" />
              {network.name}
            </li>
          ))}
        </ul>
